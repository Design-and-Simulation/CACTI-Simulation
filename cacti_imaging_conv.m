...
if load_patterns_flag
    ...
    % mask = binary_mask([sys_params.mask_size obj_num], dmd_t);    % binary mask pattern
    % mu=0.5; sigma=0.5; kernel_size=1;         % gaussian distribution params
    % mask = gray_mask([sys_params.mask_size obj_num], [mu,sigma,kernel_size]);  % gray mask pattern
    % mask_t = 0.5;
    % mask = gray_mask([sys_params.mask_size obj_num]);      % mask pattern, uniform distribution
